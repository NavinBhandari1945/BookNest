...
class _OrderDetailsPageState extends State<OrderDetailsPage> {
  ...
  Future<int> Cancel_Order({required int OrderId}) async {
    try {
      ...
      Map<String, int> OrderData = {"OrderId": OrderId};
      ...
      if (response.statusCode == 200) {
        ...
        Toastget().Toastmsg(
          "Cancel order failed.Incorrect order data format.Try again.",
        );
        return 11; // jwt error
      } else {
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              Text(
                "Discount Amount: ${widget.OrderObject.discountAmount ?? ''}",
              ),
              Text(
                "Total Price: ${widget.OrderObject.totalPrice?.toStringAsFixed(2) ?? ''}",
              ),
              ...
              Text(
                "Price: ${widget.OrderObject.price?.toStringAsFixed(2) ?? ''}",
              ),
              ...
              Text(
                "Publication Date: ${widget.OrderObject.publicationDate ?? ''}",
              ),
              ...
              Commonbutton(
                ...
                () async {
                  if (widget.OrderObject.status == "Complete") {
                    ...
                    try {
                      final Order_Cancel_Result = await Cancel_Order(
                        OrderId: widget.OrderObject.orderId!,
                      );
